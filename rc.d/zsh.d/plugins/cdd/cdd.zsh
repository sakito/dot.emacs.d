# tmux 対応版に修正を加えた物
# https://github.com/cooldaemon/myhome/blob/master/bin/cdd
#
# * cdd
...
CDD_BASE_DIR=$HOME/var/cdd
if [ ! -d "${CDD_BASE_DIR}" ];then
    mkdir -p ${CDD_BASE_DIR}
fi


if [ "$TMUX" != "" ]; then
    export CDD_PWD_FILE=$CDD_BASE_DIR/tmux.$(echo $TMUX | cut -d ',' -f 3)
    export WINDOW=$(tmux display -pt $TMUX_PANE '#I')
elif [ "$STY" != "" ]; then
