...
class Flow( object ) :
        ...
        if (kwarg.get( "closed" )) :
            ...
            if (closed_branches) :
                ...
                for e in closed_branches :
                    ...
                    if (not (description.startswith( "flow: "   ) or
                             description.startswith( "hgflow: " ) or
                             description.startswith( "hg flow," )
                             )) :
                        lines = [e.strip() for e in description.split( "\n" )]
                        self.ui.note( "  description: %s\n" % lines[0] )
                        for line in lines[1:] :
                            if (not (line.startswith( "flow: "   ) or
                                     line.startswith( "hgflow: " ) or
                                     line.startswith( "hg flow," )
                                     )) :
                                self.ui.note( "               %s\n" % lines[0] )
                        self.ui.note( "\n" )
            else :
    ...
    def _action_abort( self, stream, *arg, **kwarg ) :
        ...
        if (onstream) :
            ...
            for branch in branches :
                self._update( branch )
                self._commit( close_branch = True, message = "%sflow: Aborted %s %s." %
                              (msg, stream, branch.basename( stream, should_quote = True ),) )
            ...
            self._commit( close_branch = True, message = "%sflow: Aborted %s '%s'." %
                          (msg, stream, curr_workspace.basename( stream ),) )
    ...
    def _action_promote( self, stream, *arg, **kwarg ) :
        ...
        if (arg[1:]) :
            ...
            for dest in arg[1:] :
                ...
                self._commit( message = message + ("flow: Promoted %s '%s' (%s) to '%s'." %
                              (stream, promoted_branch.basename( stream ), short( promoted_node ), dest,)), **kwarg )
        else :
            for s in stream.destin() :
                ...
                if (trunk) :
                    ...
                    self._merge ( promoted_rev )
                    self._commit( message = message + ("flow: Promoted %s '%s' (%s) to '%s'." %
                                  (stream, promoted_branch.basename( stream ), short( promoted_node ), trunk,)), **kwarg )
                else :
    ...
    def _action_finish( self, stream, *arg, **kwarg ) :
        ...
        for s in destin_with_trunk :
            ...
            self._commit( message = "flow: Merged %s %s to %s ('%s')." % (stream, name, s, trunk,), **kwarg )
            if (s == STREAM["master"]) :
                self._tag( tag_name )
                ...
                for ss in tr_stream.destin() :
                    if (ss == develop_stream) :
                        ...
                        self._merge ( trunk   )
                        self._commit( message = "flow: Merged <develop/%s:%s> %s to %s ('%s')." %
                                      (tr_name, stream.name(), name, ss, dvtrunk,), **kwarg )
...
- upgrade  Upgrade the configuration file to v0.9.
...
when they are finished. Since version 0.9, user can create branches in
...
The new branch is named as <stream-prefix>/<name>.
""",
...
"@pull" : """
Pull from the same branch in the remote repository and update the workspace.
...
syntax:
{{{hg flow <stream> promote [branch...] [<option>...]}}}
...
"@upgrade" : """
Upgrade the configuration file from v0.7 or v0.8 to v0.9.
...
- source
  ...
  where branches in the current stream are created from. Commonly, source of a
  stream is the stream itself. But this is not always the case, for example,
  the sources of release and feature streams are the develop stream.

- destin
  Destin is another attribute of stream. The destin of a stream refers to the
  stream(s) where branches in the current stream will merge to. Commonly,
  destin of a stream is the stream itself. But this is not always the case,
...
- flow command
  Refer to other actions than those on a stream, e.g., hg flow unshelve, where
  'unshelve' is a command.
...
def _getopt( ui, key, opt ) :
    ...
    @type  opt: C{dict}
    @param opt: Raw options. Note this function mutates this object.
    
    @raise AbortFlow: AborFlow exception will be raised if no value found for an option that expects a value.
    """
    ret = {}
    for e in OPT_FILTER[key] :
        if (opt.get( e )) :
            ret[e] = opt[e]
...
"flow" :
    (flow_cmd,
     [("",  "history",   False, _("Print history of hg commands used in this workflow."),                          ),
      ...
      ("c", "commit",    False, _("Commit changes before close the branch. [finish]"),                             ),
