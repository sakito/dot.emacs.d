...
;;; WHAT'S NEW
;; 0.3: (a) zero-width matches are now visible in the visual feedback
;; (b) while constructing the replacement, the highlighting of the group matches is retained.
;; (c) internal variables/functions are prefixed with "pyregexp--"
	   ...
	   ((equal pyregexp--in-minibuffer 'pyregexp-minibuffer-replace)
	    (concat
	     ...
	     (let ((flag-infos (mapconcat 'identity 
					  (delq nil (list (when pyregexp--use-expression "using expression")
							  (when pyregexp--replace-preview "preview"))) 
					  ", ")))
	   ...
	   (lambda (output)
		...
		(let ((overlay (pyregexp--get-overlay i j)))
		  ...
		  (if (and (= 0 j) (= begin end)) ;; empty match; indicate by a pipe
		      (overlay-put overlay 'after-string (propertize "|" 'face (nth (mod i (length pyregexp--match-faces)) pyregexp--match-faces) 'help-echo "empty match"))
		    (overlay-put overlay 'after-string nil))
	 ...
	 (loop for replacement-info in replacements do 
		 ...
		 (let* ((overlay (pyregexp--get-overlay i 0))
		   ...
		   (let ((original (with-current-buffer pyregexp--target-buffer (buffer-substring-no-properties (overlay-start overlay) (overlay-end overlay)))))
		     (if (or empty-match pyregexp--replace-preview)
			 (progn 
			   (overlay-put overlay (if empty-match 'after-string 'display) (propertize (pyregexp--unescape replacement) 'face (nth (mod i (length pyregexp--match-faces)) pyregexp--match-faces)))
			   (overlay-put overlay 'priority (+ pyregexp--overlay-priority 2)))
		       (progn
			 ;;(overlay-put overlay 'display nil)
			 (overlay-put overlay 'after-string 
				      (propertize (format " => %s" (pyregexp--unescape replacement)) 'face (nth (mod i (length pyregexp--match-faces)) pyregexp--match-faces)))
			 (overlay-put overlay 'priority (+ pyregexp--overlay-priority 0))))))))
